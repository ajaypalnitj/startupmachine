...
function setupEventListeners() {
    ...
    // Compress action
    document.getElementById('start-btn').addEventListener('click', compressAllImages);
}
...
function compressImage(id) {
    return new Promise((resolve) => {
        ...
        img.onload = function() {
            ...
            image.compressedFormat = getFormatFromMimeType(mimeType);
...
// Update image result in UI after compression
function updateImageResult(image) {
    ...
    preview.src = image.compressedDataUrl;
    
    // Update the metadata
    const metaElement = imageElement.querySelector('.image-meta');
    const savingsPercentage = Math.round((1 - (image.compressedSize / image.originalSize)) * 100);
    
    metaElement.innerHTML = `
        <span>${image.compressedFormat.toUpperCase()}</span>
        <span>${formatFileSize(image.compressedSize)} (${savingsPercentage >= 0 ? '-' : '+'}${Math.abs(savingsPercentage)}%)</span>
    `;
...
function downloadImage(id) {
    ...
    // Generate filename
    const extension = `.${image.compressedFormat}`;
    const originalNameWithoutExt = image.originalName.replace(/\.[^/.]+$/, "");
    link.download = `${originalNameWithoutExt}-compressed${extension}`;
